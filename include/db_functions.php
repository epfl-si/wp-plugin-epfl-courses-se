...
function addCourseData($course){
    ...
    if(!empty($course[17])){
		...
		foreach($polyperspectives as $polyperspective) {
			...
			$polyId=0;
			switch (strtoupper($polyperspective)) {
...
function executeSQLString($sql){
	...
	if ( $wpdb->query( $sql ) === false )
	{
		return 'There was a database error when executing SQL (function executeSQL): ' . $wpdb->print_error() ."\n" . $sql;
	}
}
